...
class ImageInfo(BaseModel):
    ...
    path: Annotated[Path, Field(description="Path to the image.")]
    n_bytes: Annotated[int, Field(description="Image size in bytes")]
    status: Literal["ok", "not-found", "unreadable"]
    width: Annotated[int | None, Field(description="Image width in pixels", nullable=True)] = None
    height: Annotated[int | None, Field(description="Image height in pixels", nullable=True)] = None

    error: Annotated[
        str | None,
        Field(description="Any error encountered while trying to load the image", nullable=True),
    ] = None
...
class Annot(BaseModel):
    ...
    label: Annotated[
        str,
        Field(description="Class label of the object in the bbox, e.g. 'cow', 'cattle', 'calf' "),
    ]
    coords: Annotated[
        list[float],
        Field(
    ...
    tags: Annotated[
        dict[str, object],
        Field(description="an arbitrary dictionary of tags keyed by name", default_factory={}),
    ]
...
class AnnotationsFileInfo(BaseModel):
    ...
    byte_size: Annotated[
        int | None, Field(description="Annotation file size in bytes", nullable=True)
    ] = None
    n_lines: Annotated[int | None, Field(description="number of lines", nullable=True)] = None
    n_annots: Annotated[int, Field(None, description="number of annotations")] = 0
    annots: Annotated[
        list[Annot] | None,
        Field(default=None, description="Bounding boxes and their labels", nullable=True),
    ] = None
    error: Annotated[
        str | None,
        Field(
...
class DatasetTraits(abc.ABC):
    """Abstract class used as based below."""
...
class SkyTraits(DatasetTraits):
    ...
    def _annots_path(self, image_path: Path) -> Path:
    ...
    def _read_annots(self, annots_path: Path) -> AnnotationsFileInfo:
    ...
    def process_one(self, image_path: Path) -> AnnotatedImageRecord:
        ...
        annot_info = self._read_annots(annots_path)
...
class IcaerusTraits(DatasetTraits):
    ...
    def _read_annots(
        self, annots_path: Path, *, img_width: int, img_height: int
    ) -> AnnotationsFileInfo:
