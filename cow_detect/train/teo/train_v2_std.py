...
from torchmetrics.functional.detection import mean_average_precision, intersection_over_union
...
from cow_detect.utils.data import custom_collate_dicts, zip_dict, make_jsonifiable, make_jsonifiable_singletons
from cow_detect.utils.debug import summarize
# from cow_detect.utils.metrics import calculate_iou
from cow_detect.utils.mlflow_utils import log_params_v1
...
class TrainerV2:
    ...
    now over a standardized dataet"""
    ...
    def validate_epoch(
        ...
        with torch.no_grad():
            ...
            for batch in pbar:
                ...
                # print("prediction dicts:", summarize(predictions))
        ...
        logger.info(f"Epoch {epoch} - VALIDATION mAP/R metrics:\n{json.dumps(mapr_metrics, indent=2)}")

        avg_valid_iou = np.nanmean(valid_ious)
        non_nan_ious = (~np.isnan(valid_ious)).sum()
        ...
        logger.info(f"Epoch {epoch}: VALIDATION : {avg_valid_iou=:.4}, {non_nan_ious=}")
...
def train_v2_std_cli(
    ...
    device = device or torch.device(train_cfg.device)
    ...
    train_data_set = AnnotatedImagesDataset(
        name="train", std_data_path=train_data_path, label_to_id=label_to_id,
        limit_fraction=train_cfg.train_fraction,
    )
    valid_data_set = AnnotatedImagesDataset(
        name="valid", std_data_path=valid_data_path, label_to_id=label_to_id,
